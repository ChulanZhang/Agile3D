...
def waymo_evaluation(prediction_infos, gt_infos, class_name, sep, distance_thresh=100, fake_gt_infos=True):
    ...
    if sep:
        ...
        classes = set(pd.keys()).intersection(set(gt.keys()))
        classes = gt.keys()
        aps = {f"{['unknown', 'Vehicle', 'Pedestrian', 'Sign', 'Cyclist'][i[0]]}_L2_AP":calculate_map(*pd[i],*gt[i]) for i in classes if i in pd and i in gt}
    return aps
