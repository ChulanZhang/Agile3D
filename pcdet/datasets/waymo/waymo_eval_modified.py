...
class OpenPCDetWaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def waymo_evaluation(self, prediction_infos, gt_infos, class_name, distance_thresh=100, fake_gt_infos=True, split=False):
        ...
        if not split:
            prediction_info = [prediction_infos]
            gt_info = [gt_infos]
        else:
            prediction_info = [[info] for info in prediction_infos]
            gt_info = [[info] for info in gt_infos]
            if type(split) == str:
                prediction_info.append(prediction_infos)            
                gt_info.append(gt_infos)
        for prediction_infos, gt_infos, frame in zip(prediction_info, gt_info, range(split[1], split[1]+len(prediction_info))):
            ...
            with self.test_session(graph=graph) as sess:
                ...
                if split:
                    split[0].write(str(frame) + ":" + str(aps) + "\n")
                    split[0].flush()
        #return aps
